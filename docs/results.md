...
# Interpretation note added after Experiment 1

**Nothing above has been edited: every measured number is preserved as
originally recorded.** This note identifies claims in the text above that were
*categorical inferences beyond what Experiment 0 measured*. See
[`exp1_warmstart.md`](exp1_warmstart.md).

Experiment 0 measured target-task performance **before any adaptation**. Poor
performance before adaptation is a different quantity from poor performance
after adaptation, and several statements above silently crossed that gap.

### Claims that were not supported by Experiment 0

1. **"Training a coarse stage to convergence destroys much of what you would want
   to carry forward"** and *"that specialization actively costs it target
   performance"* (Finding 3).
   The measurement — target validation accuracy of a σ=2 model falling 9.3 pp
   from its peak while its transformed accuracy kept rising — stands. The
   inference to *destroyed transferable features* did not follow: a low
   pre-switch score does not establish that the features cannot be recovered.
   Experiment 1 shows a σ=1 model at update 1,500 scoring 0.5557 versus the
   baseline's 0.6113, then recovering to within ~1 pp of the baseline after
   adaptation. **Scope:** Experiment 1 tested an *early* warm start only, so it
   refutes the general inference, not the specific claim about *converged*
   coarse stages, which remains untested.

2. **"There is no optimization failure for a continuation method to repair"**
   (Findings 1 and "the headline problem").
   The measurement — every condition reaching ≈100 % training accuracy — stands.
   The categorical conclusion did not follow: near-perfect *final* training
   accuracy does not rule out optimization speedups at smaller budgets.
   Experiment 1's threshold diagnostic found no speedup either way, but only at
   this budget and this learning-rate schedule.

3. **"Usable σ range on 32×32 is roughly [0, 1]; σ=2 and σ=3 destroy too much
   label information to be plausible starting points — you cannot recover what
   is not there."**
   This extrapolated from pre-adaptation scores to post-adaptation usefulness.
   Experiment 1 did not test σ > 1, so this remains **untested**, not refuted.

4. **"Stages must be short… a σ=2 stage is already past its useful point by
   ~6,000 updates."**
   The peak-timing measurements stand. Treating the peak of *pre-switch* target
   accuracy as the optimal switch point was an unsupported step; the best switch
   time is a property of the adapted result, which Experiment 0 never measured.

5. **"Gaussian smoothing looks weakly negative as a family member."**
   The direction survives — Experiment 1 finds W − A = −1.17 pp and P − A =
   −0.91 pp at a matched budget — but the original reasoning leaned on claims 1
   and 3 above. The conclusion should rest on the matched-budget comparison, and
   remains scoped to this transformation, these schedules, this optimizer and
   this dataset, with three seeds.

### What Experiment 0 did establish, unchanged

The fixed-level measurements: final metrics per σ, the peak-then-decay of
pre-adaptation target accuracy, retained TV ratios and reconstruction MSE, the
descent-rate ordering of the transformed training objectives, and the
transformation-cost accounting. Those numbers are reproduced by the raw records
in `results/exp0_gaussian/` and are unaffected by this note.
