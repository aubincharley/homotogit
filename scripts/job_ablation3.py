...
``ABLATION3_JOB`` (0-3) picks the slice; the partition is deterministic.
...
from scripts.ablation3_manifest import assign, build_cells, summary
...
if __name__ == "__main__":
    job = int(os.environ.get("ABLATION3_JOB", "0"))
    n_jobs = int(os.environ.get("ABLATION3_N_JOBS", "4"))

    WORK.mkdir(parents=True, exist_ok=True)
    plan = summary()
    (WORK / "ablation3_plan.json").write_text(json.dumps(plan, indent=2))
    log("wave-3 plan: %s" % json.dumps(plan))
    ...
    log("wave-3 job %d/%d: %d cells (%s)"
        % (job, n_jobs, len(mine), [c["cell_id"] for c in mine]))
    run_job(job, mine, prefix="ablation3_job")
