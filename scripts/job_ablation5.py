...
``ABLATION5_JOB`` (0-3) picks the slice; the partition is deterministic.
...
from scripts.ablation5_manifest import assign, build_cells, summary
...
def verify_profiles(out_dir: Path) -> dict:
    """Each sigma profile must actually change the network, and differ from the others.

    This wave's silent failure mode, hit once during development: the profile was
    stored on the controller but never read by ``apply_at``, so all three arms
    would have trained as the uniform control and reported three distinct
    configurations.  Nothing raised.
    """
    import itertools
    ...
    from continuation.config import ModelConfig
    from continuation.models import build_model
    from scripts.ablation5_manifest import PROFILES

    dev = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model = build_model(ModelConfig(arch="resnet20_bn_cifar"), 10, seed=999).to(dev)
    model.eval()
    x = torch.rand(4, 3, 32, 32, device=dev)

    outs = {}
    for name, prof in [("uniform", None)] + [(k, v[0]) for k, v in PROFILES.items()]:
        ctrl = AblationController(operator="gaussian", levels=[1.0] * 30,
                                  sites=tuple(range(10)),
                                  resolution_by_epoch=[32] * 30,
                                  reduction="block2_max", placement="post_block",
                                  n_positions=10, sigma_profile=prof)
        ctrl.set_epoch(0)
        handles = attach_ablation(model, ctrl)
        with torch.no_grad():
            outs[name] = model(x).clone()
        for h in handles:
            h.remove()

    ref = outs["uniform"]
    res = {k: {"max_abs_diff_vs_uniform": float((v - ref).abs().max()),
               "differs_from_uniform": not torch.equal(v, ref)}
           for k, v in outs.items() if k != "uniform"}
    res["pairwise_distinct"] = all(not torch.equal(outs[a], outs[b])
                                   for a, b in itertools.combinations(PROFILES, 2))
    res["profiles"] = {k: v[0] for k, v in PROFILES.items()}
    res["all_ok"] = (all(v["differs_from_uniform"] for k, v in res.items()
                         if isinstance(v, dict) and "differs_from_uniform" in v)
                     and res["pairwise_distinct"])
    (out_dir / "profile_verification.json").write_text(json.dumps(res, indent=2))
    log("sigma-profile verification: %s" % ("OK" if res["all_ok"] else res))
    if not res["all_ok"]:
        raise SystemExit("a sigma profile does not act: the arm would train as "
                         "the uniform control while reporting a profile: %s" % res)
    return res


if __name__ == "__main__":
    job = int(os.environ.get("ABLATION5_JOB", "0"))
    n_jobs = int(os.environ.get("ABLATION5_N_JOBS", "4"))

    WORK.mkdir(parents=True, exist_ok=True)
    plan = summary()
    (WORK / "ablation5_plan.json").write_text(json.dumps(plan, indent=2))
    log("wave-5 plan: %s" % json.dumps(plan))

    assets = find_assets()
    verify_assets(assets, WORK)
    verify_ablation_ops(WORK)
    verify_reductions(WORK)
    verify_profiles(WORK)

    mine = assign(build_cells(), n_jobs)[job]
    log("wave-5 job %d/%d: %d cells (%s)"
        % (job, n_jobs, len(mine), [c["cell_id"] for c in mine]))
    run_job(job, mine, prefix="ablation5_job")
