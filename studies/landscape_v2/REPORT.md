...
plain → Gaussian-only barrier is the largest in every seed.

*Correction (landscape_v3 audit of `tables/interpolation_barriers.csv`).* An
earlier version also stated that the barrier between the two resolution methods
is never smaller than between plain and resolution-only. That is false: on the
train probe it is smaller in seed 1 (0.603 vs 0.869) and seed 4 (0.829 vs
0.844). See `studies/landscape_v3/tables/F_barrier_comparison_res_comb_vs_plain_res.csv`.
